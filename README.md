...
The default helper files created in RSpec Rails 3 have changed slightly. In
prior versions, only a single `spec_helper.rb` file was generated. This file
has been moved to `rails_helper.rb`. The new `spec_helper.rb` is the standard
helper generated by running `rspec --init`.
...
To switch an existing app over to use the new helpers there are two general
options depending on how you wish to proceed:

1. I need to move things over in stages:

  Create a new `rails_helper.rb` with the following content:

    ```ruby
    require 'spec_helper'
    ```

  This will allow you to continue to use the new generators, while slowly
  replacing any `spec_helper` requires in your specs. After the specs have been
  updated, you are ready to start moving the Rails specific sections out of
  `spec_helper.rb` and into `rails_helper.rb`.

2. I'm ready to just switch completely:

  - Move the existing `spec_helper.rb` to `rails_helper.rb`

    ```ruby
    git move spec/spec_helper.rb spec/rails_helper.rb
    ```

  - Run the installation rake task opting to not replace `rails_helper.rb`
  ...
  - Find/replace all instances of `require 'spec_helper'` with `require
    'rails_helper'` in any specs which rely on Rails.

### Generators

**NOTE:** Generators run in RSpec 3 will use the `rails_helper` by default. See
the above [upgrade notes](#upgrade-note) about this change.
